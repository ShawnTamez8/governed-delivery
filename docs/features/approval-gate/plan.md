...
**Hazards considered:** `docs/hazards.md` 4 (fixtures and code agreeing while both are wrong — every guard proven by breaking it; expected values come from `node:crypto` and `ARCHITECTURE.md`), 12 (configuration divergence between targets — the profile is the effective configuration, written as readable JSON). Entries 1 and 3 explicitly do **not** apply: this gate parses no model output and builds no prompt. Entries 5-7, 10, 13-14 concern stages this step does not reach.
